...
- why?
- the case of [Harold Shipman](https://en.wikipedia.org/wiki/Harold_Shipman)
- [chap1](../art_of_statistics_chap1-2.pdf)
...
- let's create two vectors of numbers

```{r}
vec1 <- sample(1:6, replace = TRUE)
vec2 <- sample(1:6, replace = FALSE)
```

- combine them into matrirx:
...
- and combine it with the vector of strings

```{r}
subj <- rep(c('subj1','subj2'), times=3)
subj
```
- and now dataframe

```{r}
df <- cbind.data.frame(subj, vec1, vec2)
df
```
- first visualisation
...
```{r}
library(ggplot2)
ggplot(df, aes(x=subj, y=vec1)) + geom_point()
```
